...
def Word2Vec_score(Question, Corpus, All_terms):
        ...
        if doc_ind==0:  #### length
           pass
            ...
            for terms1 in All_terms:
                Common_term_sum+=len(list(set(Doc_dict.keys()).intersection(terms1)))

            if Common_term_sum<15: ### (not relevant to any of the question in the batch)
                ...
                if Doc_Matrix.size==0:
                   pass
                    ...
                    for qind, ques1 in enumerate(Question):
                        if len(list(set(Doc_dict.keys()).intersection(All_terms[int(qind/4)])))==0:  ## atleast one of the 4 option has a common term
                           pass
                        else:
...
for line1 in Question_file:
    ...
    if counter%50==0:  ## taking 25 questions at a time

       Score_matrix = Word2Vec_score(All_questions, file1, All_terms)
